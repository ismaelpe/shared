...
def getPodsInfo(String namespace, String environment, String center) {
    def url = "${GlobalVars.Cloud_PRO}/api/publisher/v1/api/application/PCLD/${namespace}/environment/${environment}/az/${center}?object=POD"

    def command = buildCurlCommand("GET", url, env.Cloud_CERT, env.Cloud_PASS)

    def response = checkEndpoint.executeCurlWithRetries(command, [
        maxRetries: GlobalVars.Cloud_API_REQUEST_MAX_RETRIES,
        kpiAlmEvent: new KpiAlmEvent(
            null, null,
            KpiAlmEventStage.UNDEFINED,
            KpiAlmEventOperation.CloudAPI_HTTP_CURL, null, null, null, environment)
    ])
...
def restartPod(String componentName, String podId, String namespace, String environment, String center) {
    ...
    int retryNumber = GlobalVars.Cloud_API_REQUEST_MAX_RETRIES
    componentName = componentName.toUpperCase()

    def url = "${GlobalVars.Cloud_PRO}/api/publisher/v1/api/application/PCLD/${namespace}/component/${componentName}/environment/${environment}/az/${center}/pod/${podId}/restart"
   
    waitUntil(initialRecurrencePeriod: 15000) {
        def command = buildCurlCommand("DELETE", url, env.Cloud_CERT, Cloud_PASS)
...
def buildCurlCommand(String method, String url, def headers = [], String CERTIFICATE, String PASSWORD) {
    def cmd = "curl -k --write-out '%{http_code}' -o curlOutput.json -s " +
        "-X ${method} ${url} --connect-timeout ${GlobalVars.Cloud_API_MAX_TIMEOUT_PER_REQUEST} " +
